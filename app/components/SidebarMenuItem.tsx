...
export const SidebarMenuItem = ({ path, label, description, icon } : Props ) => {
    ...
    return (
        <Link href={ path } className={`w-full px-2 inline-flex space-x-2 items-center border-b border-slate-700 py-3 ${ currentPath === path ? 'bg-purple-600' : '' } hover:bg-white/5 transition ease-linear duration-150`}>
            <div>
                { icon }                      
            </div>
