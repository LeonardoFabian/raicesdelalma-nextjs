import { ChangeEvent } from "react";
import Image from "next/image";

interface Props {
  name: string;
  value?: string;
  label?: string;
  onChange?:
    | ((e: ChangeEvent<HTMLInputElement>) => void | undefined)
    | undefined;
}

export const FileInput = ({ name, label, value, onChange }: Props) => {
  return (
    <div className="relative z-0 w-full mb-5 group ">
      <input
        type="file"
        name={name}
        id={name}
        className="block py-2.5 px-0 w-full hover:cursor-pointer text-base text-gray-900 bg-transparent border-0 border-b-2 border-gray-300 appearance-none dark:text-white dark:border-gray-600 dark:focus:border-primary focus:outline-none focus:ring-0 focus:border-primary peer"
        onChange={onChange}
      />
      {label && (
        <label
          htmlFor={name}
          className="peer-focus:font-medium absolute text-base text-gray-500 dark:text-gray-400 duration-300 transform -translate-y-6 scale-75 top-3 -z-10 origin-[0] peer-focus:start-0 rtl:peer-focus:translate-x-1/4 rtl:peer-focus:left-auto peer-focus:text-primary peer-focus:dark:text-primary peer-placeholder-shown:scale-100 peer-placeholder-shown:translate-y-0 peer-focus:scale-75 peer-focus:-translate-y-6"
        >
          {label ?? ""}
        </label>
      )}
      {value && (
        <Image
          className="w-full h-44 object-contain mt-4 py-4 bg-gray-200 rounded-lg"
          src={`/uploads/${value}`}
          alt={name}
          width={350}
          height={150}
          priority={false}
        />
      )}
    </div>
  );
};
