...
import { GiftMessageQR } from "./GiftMessageQR";
...
export const OrdersTable = ({ orders }: Props) => {
  ...
  const handleOpenQrModal = (order: IOrder) => {
    setOrder(order);
  ...
  return (
    <>
                  ...
                  <td className="px-6 py-4 whitespace-nowrap text-sm md:text-base font-medium text-text-primary">
                    <div className="flex items-center justify-between gap-3">
                      ...
                      {order?.giftMessage && (
                        ...
                        >
                          <IoQrCode size={24} className="cursor-pointer" />
                        </button>
                      )}
                    </div>
                  </td>
                  ...
                  <td className="text-sm md:text-base text-text-primary px-6 text-right">
                    <div className="flex items-center justify-end gap-4">
                      ...
                      {order.giftMessage && session?.user.role === "admin" && (
                        <button
                          onClick={() => handleOpenQrModal(order)}
                          className="text-link font-semibold hover:underline whitespace-nowrap cursor-pointer"
                        >
                          Generate QR
                        </button>
                      )}
      ...
      {isQrModalOpen && order && order.giftMessage && (
        <Modal isOpen={isQrModalOpen} onClose={() => setIsQrModalOpen(false)}>
          <GiftMessageQR messageId={order.giftMessage.id!} orderId={order.id} />
        </Modal>
      )}
    </>
  );
